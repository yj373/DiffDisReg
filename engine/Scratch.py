...
def scratch_train(args):
    ...
    tokenizer, noise_scheduler, text_encoder_cls, text_encoder, vae, unet, repo_id = model_prepare(args, accelerator)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                # print(">>> ", model_input.shape, seg_values.shape) # torch.Size([2, 4, 64, 64]) torch.Size([2, 1, 512, 512])
                ...
                # print(">>>>", timesteps.shape) # torch.Size([2]) 2 times batch size if PP
                ...
                if not args.with_mse_snr:
                    ...
                    base_weight = (
                        torch.stack([snr, args.mse_snr_gamma * torch.ones_like(timesteps)], dim=1).min(dim=1)[0] / snr
                    )
                ...
                if args.with_attention_reg:
                    select = [args.attention_reg_token_idx1] if args.attention_reg_token_idx2 < 0 else [args.attention_reg_token_idx1, args.attention_reg_token_idx2] # 2: sks, 3: dog 
                    # prior_select = [args.attention_reg_prior_token_idx] # 2: dog
